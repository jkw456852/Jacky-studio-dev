...
export const SYSTEM_ANNOUNCEMENTS: SystemAnnouncement[] = [
  {
    ...
    summary:
      "这一轮收的不是单点小修，而是把工作台几条高频使用链路一起补顺了。第一块是安卓平板触控：现在画布已经支持双指缩放和平移、单指拖节点、空白区框选、端口触控连线，mark 也能直接点按落点，连线还能先点中再断开。第二块是图片节点尺寸：新导入的图、引用上传后的图、生成完成回写的图、Agent 回填到画布的图，都会尽量按原图比例落板，不再都挤成同一种固定尺寸。第三块是可见性和理解链路：生成正式排队时会补一条更明确的 request 日志，最近活动读取也更容易按 requestId / elementId 回看；主脑收到图文混排和研究上下文时，也会比以前更容易理解你这一轮到底是在问、在查，还是在让它继续执行。",
    features: [
    ...
    fixes: [
      ...
      "修复了主脑在图文混排、已上传附件和研究上下文并存时更容易误判输入结构的问题，本轮会带上更完整的 inlineParts 和多模态元数据。",
    ],
    experiments: [
  ...
  },
  {
    ...
    experiments: [
      "主脑对图文混排消息的理解链路目前已确认按 input block 顺序发送，后面还会继续观察更多连续插图、多 mark、多轮追问下的稳定性。",
    ],
  },
  {
