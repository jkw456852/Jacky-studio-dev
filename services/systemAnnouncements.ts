...
export const SYSTEM_ANNOUNCEMENTS: SystemAnnouncement[] = [
  {
    id: "2026-06-18-workspace-frontstage-sidebar-and-search-grounding-refresh-2",
    date: "2026.06.18 18:55",
    title: "工作台这一轮继续往前台产品收口了：Skill 更像真功能，搜索引用也开始更像真依据",
    summary:
      "这轮不是只改文案或外观，而是继续把侧边栏对话区、Skill 入口、发送链路、联网检索和引用呈现一起收实。底部输入区和 Skill Book 更接近前台聊天产品，角色 / Skill 的状态会更稳定地跟随当前会话；联网检索不再先于智能体抢跑，更多时候会改成先理解问题、再决定要不要搜。搜索结果展示也从“看起来像有引用”继续往“真的能解释这句话依据了哪些来源”推进，至少把来源提取、展开方式和句内引用这条链路补到了更可验证的状态。顺手还补了一批之前影响使用感的小崩溃和默认值问题，比如 MessageList 漏导入图标、Skill 面板初始化顺序、关键词节点风格库默认偷偷启用等。",
    features: [
      "侧边栏底部输入区、模式选择、Skill Book 和模型偏好继续往 Lovart 式前台结构收口，减少设置面板感，让模式、技能和输入动作更像同一套前台交互。",
      "Skill Book 现在更明确区分前台快捷 Skill、用户自定义 Skill 和执行方式入口，后续做角色 / Skill 治理时不需要再从一堆散状态里反推。",
      "联网回复里的引用展示继续前移：支持更自然的句内编号、来源浮层和摘录展开，阅读时更容易判断当前结论到底挂靠了哪条搜索结果。",
      "工作台搜索链路补上了更完整的提取流程，网页搜索、原文提取和结果组装之间的状态比以前更可见，后续继续查天气、参数页和资料页时更容易定位到底卡在搜索、提取还是回答阶段。",
    ],
    fixes: [
      "修复了 InputAreaBottomToolbar 因 mySkillEntries 初始化顺序错误导致整个底栏直接崩溃的问题，Skill 面板现在不会再因为 before initialization 报错白屏。",
      "修复了 MessageList 使用 Sparkles 图标却未导入的运行时错误，聊天区在打字态 / 进行态下不会再因为图标未定义直接炸掉。",
      "修复了新建关键词节点会默认偷偷带上风格库约束的问题；现在新节点默认关闭风格库，旧节点空值也不会再被回退成 default。",
      "修复了风格库显示层和实际生图执行层的默认值不一致问题，避免出现“界面看着关了，生成时其实还在按默认风格库跑”的错位。",
      "继续修正了 mark / 裁切引用坐标链路的对齐问题，局部引用在输入区和消息里预览时更接近用户实际点选区域。",
    ],
    experiments: [
      "句内引用目前已经比上一版更像真实依据，但还没有彻底做到“每一句都可自动精确落到对应网页段落”；后面还会继续补来源到句子的更细粒度映射。",
      "角色和 Skill 的前台化已经做了第一轮收口，下一步会继续做功能审计，决定哪些能力应该保留成角色，哪些应该并到 Skill，哪些该直接下沉成自动能力。",
    ],
  },
  {
