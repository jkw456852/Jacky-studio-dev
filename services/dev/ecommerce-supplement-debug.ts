...
type EcommerceSupplementDebugRecord = {
  recordedAt: string;
  stage: string;
  payload: unknown;
};
...
function getDailyLogPath(): string {
  return path.join(ensureRootDir(), `ecommerce-supplement-debug-${getTodayKey()}.jsonl`);
}

function getLatestSnapshotPath(): string {
  return path.join(ensureRootDir(), "latest-ecommerce-supplement-debug.json");
}
...
function cleanupOldFiles(): void {
  ...
  const files = fs
    .readdirSync(rootDir)
    .filter((name) => /^ecommerce-supplement-debug-\d{8}\.jsonl$/.test(name))
...
function normalizeRecord(body: any): EcommerceSupplementDebugRecord {
...
function writeRecord(record: EcommerceSupplementDebugRecord) {
...
export function apiEcommerceSupplementDebugPlugin(): Plugin {
  return {
    name: "vite-plugin-api-ecommerce-supplement-debug",
    configureServer(server) {
      server.middlewares.use("/api/debug-ecommerce-supplement", async (req, res) => {
