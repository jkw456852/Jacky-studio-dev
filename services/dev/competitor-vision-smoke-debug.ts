...
type CompetitorVisionSmokeDebugRecord = {
  recordedAt: string;
  stage: string;
  payload: unknown;
};
...
function getDailyLogPath(): string {
  return path.join(
    ensureRootDir(),
    `competitor-vision-smoke-debug-${getTodayKey()}.jsonl`,
  );
}

function getLatestSnapshotPath(): string {
  return path.join(ensureRootDir(), "latest-competitor-vision-smoke-debug.json");
}
...
function cleanupOldFiles(): void {
  ...
  const files = fs
    .readdirSync(rootDir)
    .filter((name) => /^competitor-vision-smoke-debug-\d{8}\.jsonl$/.test(name))
...
function normalizeRecord(body: any): CompetitorVisionSmokeDebugRecord {
...
function writeRecord(record: CompetitorVisionSmokeDebugRecord) {
...
export function apiCompetitorVisionSmokeDebugPlugin(): Plugin {
  return {
    name: "vite-plugin-api-competitor-vision-smoke-debug",
    configureServer(server) {
      server.middlewares.use("/api/debug-competitor-vision-smoke", async (req, res) => {
