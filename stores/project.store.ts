...
const compactDesignSessionForPersist = (session: DesignSessionState): DesignSessionState => ({
  ...
  consistencyCheckEnabled: session.consistencyCheckEnabled === true,
  preGenerationPlanningEnabled: session.preGenerationPlanningEnabled !== false,
...
const initialState: Omit<ProjectState, 'actions'> = {
  ...
  designSession: {
    ...
    preGenerationPlanningEnabled: true,
    referenceSummary: '',
    constraints: [],
    forbiddenChanges: [],
    approvedAssetIds: [],
