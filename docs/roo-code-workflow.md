...
- 用户举例时，先抽象原理，再实现机制；禁止把例子本身直接做成硬编码
...
- 用户用“比如”“例如”“像这样”举例时，默认动作不是照抄例子，而是先提炼成可复用原则、状态结构或决策流程，再落代码

## 持续推进规则

- 任何一轮待办的最后一个任务，必须固定写成“基于当前现状自动安排并切入下一轮待办”。
- 禁止把最后一个任务写成纯收尾、纯总结、纯文档同步或停机式任务。
- 每完成一轮后，必须先更新当前进度，再根据代码现状生成下一轮任务，并立刻把其中第一项切成进行中。
- 如果中途出现分叉决策，默认由执行者基于现状自行选择并继续，不把“等确认”写成默认终点。
