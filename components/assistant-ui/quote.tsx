...
const QuoteBlockImpl = ({ text }: { text: string; messageId: string }) => {
  return (
    <QuoteBlockRoot>
      <QuoteBlockIcon />
...
const QuoteBlock = memo(QuoteBlockImpl);
...
function SelectionToolbarRoot({
  ...
  return (
    <SelectionToolbarPrimitive.Root
      ...
      className={cn("bg-popover flex items-center gap-1 rounded-lg border px-1 py-1 shadow-md", className)}
      {...props}
    />
  );
}
...
function SelectionToolbarQuote({
  ...
  return (
    <SelectionToolbarPrimitive.Quote
      ...
      className={cn("text-popover-foreground hover:bg-accent flex items-center gap-1.5 rounded-md px-2.5 py-1 text-sm transition-colors", className)}
      {...props}
    >
      {children ?? (
        <>
...
const SelectionToolbar = memo(SelectionToolbarImpl);
...
const ComposerQuotePreview = memo(ComposerQuotePreviewImpl);
