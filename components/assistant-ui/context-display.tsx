...
const useContextDisplay = (): ContextDisplayContextValue => {
  const context = useContext(ContextDisplayContext);
  if (!context) {
...
const ContextDisplayRootBase: FC<{
  modelContextWindow: number;
  children: ReactNode;
  usage: ThreadTokenUsage | undefined;
}> = ({ modelContextWindow, children, usage }) => {
  ...
  useEffect(() => {
    setTokenState((previous) => {
      ...
      if (rawTokens > 0 && rawTokens !== previous.totalTokens) {
        return { ...previous, totalTokens: rawTokens, usage };
      }
  ...
  const contextValue = useMemo(
    ...
    [modelContextWindow, percent, tokenState.usage, totalTokens],
  );
...
const ContextDisplayRootInternal: FC<{
  modelContextWindow: number;
  children: ReactNode;
}> = ({ modelContextWindow, children }) => {
...
const ContextDisplayRoot: FC<ContextDisplayRootProps> = (props) => {
...
const ContextDisplayTrigger: FC<React.ComponentProps<"button">> = ({
  className,
  children,
  ...props
}) => (
...
const ContextDisplayContent: FC<{
  side?: "top" | "bottom" | "left" | "right";
  className?: string;
}> = ({ side = "top", className }) => {
    ...
    >
      <div className="text-xs">
        <div className="flex items-baseline justify-between gap-6 whitespace-nowrap">
          <span className="font-medium">上下文使用量</span>
